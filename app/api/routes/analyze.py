...
# ✅ Lazy singletons — instantiated on first request, never at import time
@lru_cache()
def get_ner() -> MedicalNERPipeline:
...
async def analyze_text(
    ...
    timer = LatencyTimer()
    ner = get_ner()           # ✅ lazy singleton
    summarizer = get_summarizer()  # ✅ lazy singleton
    ...
    if not looks_medical(request.text):
        raise HTTPException(status_code=422, detail=MEDICAL_REJECTION_MESSAGE)
...
async def analyze_uploaded_file(
    ...
    if not text or len(text.strip()) < 20:
        raise HTTPException(
            status_code=422,
            detail=f"Could not extract text from file (got {len(text)} chars). The PDF may be image-based or corrupted."
        )
    ...
    return await analyze_text(req, rag=rag, tracker=tracker)

@router.delete("/clear")
async def clear_documents(rag: MedicalRAGPipeline = Depends(get_rag)):
    rag.clear_documents()
